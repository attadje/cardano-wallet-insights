...
})({"8Gjf3":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "cf892df363443e7f";
...
},{}],"2j9L2":[function(require,module,exports) {
/*
 ...
 */ /* global define:false, require: false, jQuery:false */ (function(factory) {
    ...
    String.format = function() {
        ...
        return str1.replace(/(\{\{\d\}\}|\{\d\})/g, function(str) {
    ...
    function isDuplicateNotification(notification) {
        ...
        $('[data-notify="container"]').each(function(i, el) {
            ...
            if (isSameTitle && isSameMsg && isSameType) //we found the dupe. Set the var and stop checking.
            isDupe = true;
            return !isDupe;
        });
        return isDupe;
    ...
    function Notify(element, content, options) {
        ...
        if (this.settings.content.target === "-") this.settings.content.target = this.settings.url_target;
        this.animations = {
            start: "webkitAnimationStart oanimationstart MSAnimationStart animationstart",
            end: "webkitAnimationEnd oanimationend MSAnimationEnd animationend"
        };
        if (typeof this.settings.offset === "number") this.settings.offset = {
    ...
    $.extend(Notify.prototype, {
        init: function() {
            var self = this;
            this.buildNotify();
            if (this.settings.content.icon) this.setIcon();
            ...
            this.notify = {
                ...
                update: function(command, update) {
                    ...
                    if (typeof command === "string") commands[command] = update;
                    else commands = command;
                    for(var cmd in commands)switch(cmd){
                        case "type":
                        ...
                        case "icon":
                            ...
                            if (self.settings.icon_type.toLowerCase() === "class") $icon.removeClass(self.settings.content.icon).addClass(commands[cmd]);
                            else {
                                if (!$icon.is("img")) $icon.find("img");
        ...
        buildNotify: function() {
            ...
            if (!this.settings.allow_dismiss) this.$ele.find('[data-notify="dismiss"]').css("display", "none");
            if (this.settings.delay <= 0 && !this.settings.showProgressbar || !this.settings.showProgressbar) this.$ele.find('[data-notify="progressbar"]').remove();
        },
        setIcon: function() {
            this.$ele.addClass("alert-with-icon");
            if (this.settings.icon_type.toLowerCase() === "class") this.$ele.find('[data-notify="icon"]').addClass(this.settings.content.icon);
            else if (this.$ele.find('[data-notify="icon"]').is("img")) this.$ele.find('[data-notify="icon"]').attr("src", this.settings.content.icon);
            else this.$ele.find('[data-notify="icon"]').append('<img src="' + this.settings.content.icon + '" alt="Notify Icon" />');
        },
        ...
        placement: function() {
            ...
            if (this.settings.newest_on_top === true) offsetAmt = this.settings.offset.y;
            ...
            this.$ele.one(this.animations.start, function() {
                ...
                if ($.isFunction(self.settings.onShown)) self.settings.onShown.call(this);
            });
            setTimeout(function() {
                if (!hasAnimation) {
                    if ($.isFunction(self.settings.onShown)) self.settings.onShown.call(this);
        ...
        bind: function() {
            ...
            if ($.isFunction(self.settings.onClick)) this.$ele.on("click", function(event) {
                if (event.target != self.$ele.find('[data-notify="dismiss"]')[0]) self.settings.onClick.call(this, event);
            });
        ...
        close: function() {
            ...
            self.reposition(posX);
            if ($.isFunction(self.settings.onClose)) self.settings.onClose.call(this.$ele);
            this.$ele.one(this.animations.start, function() {
                hasAnimation = true;
            }).one(this.animations.end, function() {
                $(this).remove();
                if ($.isFunction(self.settings.onClosed)) self.settings.onClosed.call(this);
            });
            setTimeout(function() {
                if (!hasAnimation) {
                    self.$ele.remove();
                    if (self.settings.onClosed) self.settings.onClosed(self.$ele);
                }
            }, 600);
        },
        reposition: function(posX) {
            ...
            if (this.settings.newest_on_top === true) $elements = this.$ele.prevAll(notifies);
    ...
    $.notifyClose = function(selector) {
        if (typeof selector === "undefined" || selector === "all") $("[data-notify]").find('[data-notify="dismiss"]').trigger("click");
        else if (selector === "success" || selector === "info" || selector === "warning" || selector === "danger") $(".alert-" + selector + "[data-notify]").find('[data-notify="dismiss"]').trigger("click");
        else if (selector) $(selector + "[data-notify]").find('[data-notify="dismiss"]').trigger("click");
        else $('[data-notify-position="' + selector + '"]').find('[data-notify="dismiss"]').trigger("click");
    };
    $.notifyCloseExcept = function(selector) {
        if (selector === "success" || selector === "info" || selector === "warning" || selector === "danger") $("[data-notify]").not(".alert-" + selector).find('[data-notify="dismiss"]').trigger("click");
...
},{}]},["8Gjf3","2j9L2"], "2j9L2", "parcelRequire3ffb")

//# sourceMappingURL=index.63443e7f.js.map
